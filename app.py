...
def carregar_e_treinar_dados():
    # 1. Download do Kaggle (Silencioso e sem chave)
...
with aba3:
    ...
    if st.button("Executar Teste de Modelos (Benchmark)"):
        with st.spinner("Treinando e comparando algoritmos..."):
            ...
            # 2. Árvore (Linha corrigida!)
            reg_tree = DecisionTreeRegressor().fit(X_train, y_train)
            mae_tree = mean_absolute_error(y_test, reg_tree.predict(X_test))
            
            # 3. Random Forest (Já treinado)
            mae_rf = mean_absolute_error(y_test, modelo_principal.predict(X_test))
            
            # Padronização para SVR e Redes Neurais
            ...
            # 4. SVR
            reg_svr = SVR(kernel='rbf').fit(X_train_scaled, y_train_scaled.ravel())
            prev_svr = scaler_y.inverse_transform(reg_svr.predict(X_test_scaled).reshape(-1, 1))
            mae_svr = mean_absolute_error(y_test, prev_svr)
            
            # 5. Redes Neurais (MLP)
            reg_mlp = MLPRegressor(max_iter=1000, hidden_layer_sizes=(9,9)).fit(X_train_scaled, y_train_scaled.ravel())
            prev_mlp = scaler_y.inverse_transform(reg_mlp.predict(X_test_scaled).reshape(-1, 1))
            mae_mlp = mean_absolute_error(y_test, prev_mlp)
            
            # Exibindo os resultados
            resultados = pd.DataFrame({
                "Algoritmo": ["Redes Neurais (MLP)", "Random Forest", "SVR", "Regressão Linear", "Árvore de Decisão"],
                "MAE (Erro Médio)": [mae_mlp, mae_rf, mae_svr, mae_lin, mae_tree]
            }).sort_values(by="MAE (Erro Médio)")
            
            st.dataframe(resultados, use_container_width=True)
            st.info("Nota: O modelo adotado no simulador (Aba 2) é o Random Forest, que apresenta um excelente custo-benefício computacional e alta precisão para a realidade do produtor.")
